...
WANPROJ_opt = loaded_arr.reshape(loaded_arr.shape[0], loaded_arr.shape[1] // N_w, N_w)

loaded_arr = np.loadtxt("WANPROJ.txt").view(complex)
WANPROJ = loaded_arr.reshape(loaded_arr.shape[0], loaded_arr.shape[1] // N_w, N_w)

#%%

PROJ = open('wannier90.eig', "r")
PROJ = PROJ.read()
...
for k in range(N_k):
    for b in range(N_b):
            E[k, b] = float(PROJ[2+3*b+3*N_b*k])µ
